...
<h1 class="mt-4">Buscador {{ env('APP_NAME') }}</h1>
    <ol class="breadcrumb mb-4">
        <li class="breadcrumb-item active">Resultados para: "<b>{{ app('request')->input('searchFor') }}</b>"</li>
    </ol>

	@if ($users->total() == 0 && $payments->total() == 0 && $files->total() == 0 && $cursos->total() == 0)
	<div class="row">
		<div class="col d-flex justify-content-center py-3">
			<div>
				<div class="d-flex justify-content-center py-4">
					<img src="/images/empty_state.gif" alt="No results" height="200" width="200">
				</div>
				<h5 class="text-muted text-center">SIN RESULTADOS</h5>
				<p class="text-muted">No hemos encontrado resultados para la búsqueda</p>
			</div>
		</div>
	</div>
	@endif
		...
		<div class="col-12">   
			...
			<div class="d-md-flex align-items-center mb-3">
				<div>
					...
					<p class="card-subtitle">Usuarios</p>
				</div>
			</div>
			<!-- title -->
				...
				<table class="table table-sm v-middle">
					<thead>
						<tr class="bg-light">
							...
							<th class="border-top-0">DNI / CUIT</th>
					...
					<tbody>
						...
						<tr>
							...
							<td>{{ $user->fullname() }}</td>
							<td>{{ $user->email }}</td>
							<td>{{ $user->documento_nro ? $user->documento_nro  : '-' }} / {{ $user->cuit ? $user->cuit : '-' }}</td>
							<td>{{ $user->telefono }}</td>
							<td></td>
						</tr>
						@endforeach
			...
			<div class="d-md-flex align-items-center my-3">
				<div>
					<p class="card-subtitle">Multimedia</p>
				</div>
			</div>
			<!-- title -->
			<div class="table-responsive">
				<table class="table table-sm v-middle">
					...
					<tbody>
						...
						<tr>
							...
							<td><a href="{{Storage::disk('uploads')->url($file->path)}}" target="_blank">{{ (strlen($file->name) > 15) ? substr($file->name , 0, 25).'...' : $file->name}}</a></td>
			...
			<div class="d-md-flex align-items-center my-3">
				<div>
					<p class="card-subtitle">Cursos</p>
				</div>
			</div>
			<!-- title -->
			<div class="table-responsive">
				<table class="table table-sm v-middle">
					...
					<tbody>
						...
						<tr>
							<td> <a href="{{ route('cursos.edit', $curso->id) }}">{{ $curso->titulo }}</a></td>
							<td>@truncateString(['$curso->descripcion', 30])</td>
